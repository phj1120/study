...
public class UserService {
    ...
    public UserDto signup(UserDto userDto) {
        if (userRepository.findOneWithAuthoritiesByAuthId(userDto.getUsername()).orElse(null) != null) {
            throw new DuplicateMemberException("이미 가입되어 있는 유저입니다.");
        }

        User user = User.builder()
                .authId(userDto.getUsername())
        ...
        authorityRepository.save(new Authority(user, AuthorityType.ROLE_USER));
    ...
    public UserDto getUserWithAuthorities(String username) {
        return UserDto.from(userRepository.findOneWithAuthoritiesByAuthId(username).orElse(null));
    }
    ...
    public UserDto getMyUserWithAuthorities() {
        return UserDto.from(
                SecurityUtil.getCurrentUsername()
                        .flatMap(userRepository::findOneWithAuthoritiesByAuthId)
